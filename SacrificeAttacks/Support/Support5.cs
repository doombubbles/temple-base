...
public class Support5 : TempleBaseSacAttack
{
    ...
    protected override IEnumerable<TowerBehaviorModel> GetBehaviors()
    {
        ...
        yield return GetSacrificeEffect(SunGod, 30001).GetDescendant<PierceSupportModel>().FixedSacSupport();
        yield return GetSacrificeEffect(SunGod, 30001).GetDescendant<RangeSupportModel>().FixedSacSupport();
        if (!TempleBaseMod.NerfedIncome)
        {
            yield return GetSacrificeEffect(SunGod, 45001).GetDescendant<PerRoundCashBonusTowerModel>().Duplicate();
        }
        yield return GetSacrificeEffect(SunGod, 45001).GetDescendant<RateSupportModel>().FixedSacSupport();
